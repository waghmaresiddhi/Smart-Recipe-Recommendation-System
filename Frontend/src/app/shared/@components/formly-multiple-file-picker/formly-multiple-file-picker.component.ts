...
@Component({
  selector: 'app-formly-multi-file-picker',
  template: `
      <div class="w-full mx-auto">
        <input formly-multiple-file-picker #multiFileInput type="file"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mt-4">
          <div *ngFor="let file of selectedFiles;let i = index" class="profile-wrap bg-white shadow-md " >
            <mat-icon (click)="deleteSingleImage(file.id)" class="bg-red-500 edit-icon" svgIcon="heroicons_solid:trash" matPrefix>
            </mat-icon>
            <p>File: <a target="_blank" style="word-break:break-word" [href]="file.url">{{file.url | urlToFilename}}</a></p>
          </div>
        </div>
    </div>
  `,
...
export class FormlyMultipleFilePickerComponent extends FieldType implements OnInit {
